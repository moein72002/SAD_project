...
# services["total_cost"] = pd.to_numeric(services["total_cost"], errors='coerce')
# services[column_to_remove] = services[column_to_remove] / 60000
column_name = "total_cost"
services[column_name] = services[column_name].str.replace(',', '').astype(int)

updated_address = './backend/generated_data/update_name_csv_files'

services.to_csv(f'{updated_address}/Services.csv', index=False)
...
drugs['drug_id'] = [(counter + 1) for counter in range(len(drugs))]
...
drugs.to_csv(f'{updated_address}/Drugs.csv', index=False)

# Generate Charity data
charities = []
charity_ids = []
counter = 0
top_5_cities = sorted(locations, key=lambda x: x['sum_of_population'], reverse=True)[:5]
...
for counter in range(500):
    doctors.append({
        ...
        'password': hash_password(fake.password()),
        'address': fake.address(),
        'location_id': random.choice(top_5_cities)['id'],
        'createdAt': fake.date_between(start_date= date.today() - timedelta(days=6*30), end_date= date.today() - timedelta(days=2*30)),
        'updatedAt': fake.date_between(start_date= date.today() - timedelta(days=2*30), end_date= date.today() - timedelta(days=1*30))
...
for free_time in random.sample(doctor_free_times, int(len(doctor_free_times) * 0.9)):
    ...
    doctor_visits.append({
        'id': counter,
        'doctor_free_time_id': free_time['id'],
        'patient_id': random.choice(patient_ids),
        'service_id': services['id'].sample().iloc[0],  # Assuming 100 services
        'tooth_number': random.randint(1, 32),
        'is_reminded': fake.boolean()
    })
    doctor_visit_ids.append(counter)
...
for visit in random.sample(doctor_visits, int(len(doctor_visits) * 0.3)):
    ...
    radiology_center_visits.append({
        'id': counter + 1,
        'radiology_center_free_time_id': free_time['id'],
...
for counter in range(int(0.8 * len(doctor_visit_ids))):
    prescriptions.append({
        ...
        'drug_id' : drugs['drug_id'].sample().iloc[0]
    })
...
write_csv('Admins.csv', ['id', 'first_name', 'last_name', 'national_id', 'phone_number', 'email', 'password'], admins)
write_csv('Charities.csv', ['id', 'name', 'phone_number', 'address', 'location_id'], charities)
write_csv('CharityEmployees.csv', ['id', 'charity_id', 'first_name', 'last_name', 'national_id', 'phone_number', 'email', 'password'], charity_employees)
write_csv('Doctors.csv', ['id', 'first_name', 'last_name', 'doctor_code', 'major', 'phone_number', 'email', 'password', 'address', 'location_id','createdAt','updatedAt'], doctors)
write_csv('DoctorFreeTimes.csv', ['id', 'doctor_id', 'date', 'start_time', 'finish_time'], doctor_free_times)
write_csv('DoctorVisits.csv', ['id', 'doctor_free_time_id', 'patient_id', 'service_id', 'tooth_number', 'is_reminded'], doctor_visits)
write_csv('Patients.csv', ['id', 'charity_id', 'first_name', 'last_name', 'national_id', 'birthdate', 'sex','createdAt','updatedAt'], patients)
write_csv('RadiologyCenters.csv', ['id', 'name', 'code', 'phone_number', 'address', 'location_id'], radiology_centers)
write_csv('RadiologyCenterEmployees.csv', ['id', 'radiology_center_id', 'first_name', 'last_name', 'national_id', 'phone_number', 'password'], radiology_center_employees)
write_csv('RadiologyCenterFreeTimes.csv', ['id', 'radiology_center_id', 'date', 'start_time', 'finish_time'], radiology_center_free_times)
write_csv('RadiologyCenterVisits.csv', ['id', 'radiology_center_free_time_id', 'doctor_visit_id', 'opg_image', 'is_reminded'], radiology_center_visits)
write_csv('SystemSecretaries.csv', ['id', 'first_name', 'last_name', 'national_id', 'phone_number', 'email', 'password'], system_secretaries)
write_csv('Prescriptions.csv',['id', 'doctor_visit_id', 'drug_id'], prescriptions)
